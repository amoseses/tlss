...
export function RecentlyViewedRail({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={compact ? "rounded-3xl border border-border/70 bg-white p-4" : "givit-section"}>
      <div className="mb-3 flex items-center gap-2 text-sm font-bold text-givit-ink"><Clock className="h-4 w-4 text-givit-ember" /> Recently viewed</div>
        ...
        {items.slice(0, 6).map((item) => (
          <Link key={item.slug} href={item.href} className="group rounded-2xl border border-border/60 bg-white p-2 transition hover:border-givit-ember/40 hover:shadow-sm">
            <div className="relative aspect-square overflow-hidden rounded-xl bg-givit-sand">
              {item.image ? <Image src={item.image} alt="" fill className="object-cover transition group-hover:scale-105" unoptimized /> : <div className="flex h-full items-center justify-center text-2xl">🎁</div>}
            </div>
