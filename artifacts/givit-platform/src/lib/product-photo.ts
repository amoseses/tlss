 ...
 * shots. Grouped by marketplace category so a fallback for a tech product
 * actually lands on a tech-ish photo instead of landing anywhere in the
 * whole pool by pure hash coincidence (which could just as easily put a
 * pets photo on a tech product) — that mismatch was the main driver behind
 * "the pictures don't match the gift ideas" feedback. Each list is kept
 * large enough that the deterministic hash in productPhotoFallback() still
 * spreads products across several distinct photos within their category.
 */
const FALLBACK_PHOTOS_BY_CATEGORY: Record<string, string[]> = {
  books: [
...
const ALL_FALLBACK_PHOTOS = Object.values(FALLBACK_PHOTOS_BY_CATEGORY).flat();

export function productPhotoFallback(seed: string, categorySlug?: string | null) {
  const pool = (categorySlug && FALLBACK_PHOTOS_BY_CATEGORY[categorySlug]) || ALL_FALLBACK_PHOTOS;
  let hash = 0;
...
export function resolveProductImageSrc(productId: string, images: ProductImage[], categorySlug?: string | null) {
  const sorted = [...images].sort((a, b) => a.sort_order - b.sort_order);
  const firstUsable = sorted.find((image) => image.storage_path && !looksAiGenerated(image.storage_path));
  if (!firstUsable) return productPhotoFallback(productId, categorySlug);
