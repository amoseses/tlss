  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      {GIFT_COLLECTIONS.map((collection) => (
        <Link key={collection.slug} href={`/products?q=${encodeURIComponent(collection.query)}`} className="rounded-2xl border border-border/70 bg-white p-5 transition hover:-translate-y-1 hover:border-givit-ember/40 hover:shadow-md">
