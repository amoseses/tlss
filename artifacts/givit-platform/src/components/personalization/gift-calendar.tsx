  ...
  return (
    <section className="givit-section">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-givit-ember">
            <CalendarDays className="h-4 w-4" /> Personal gift calendar
          </p>
        ...
        {next ? (
          <div className="rounded-2xl bg-givit-sand p-4 text-sm text-givit-ink">
            ...
            <p className="mt-1 text-muted-foreground">
              Your {next.name}&apos;s {next.type.toLowerCase()} is in {daysUntil(next.date)} days.
            </p>
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
        {sorted.map((event) => (
          <div key={event.id} className="rounded-2xl bg-card p-4 shadow-sm shadow-black/[0.03] transition-shadow duration-200 hover:shadow-md">
            <div className="flex items-start justify-between gap-3">
              ...
              <button type="button" className="text-muted-foreground hover:text-destructive" onClick={() => save(events.filter((item) => item.id !== event.id))}>
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
            <Button asChild variant="outline" size="sm" className="mt-3 rounded-full">
              <Link href={`/gift?prompt=${encodeURIComponent(`Gift for ${event.name}, ${event.type}, likes ${event.interests}`)}`}>
                Get 3 options
              </Link>
            </Button>
          </div>
        ...
        {sorted.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-givit-ember/30 bg-givit-sand/40 p-4 text-sm text-muted-foreground">
            Add a first reminder to unlock monthly gift alerts.
          </p>
        ) : null}
      </div>
    </section>
