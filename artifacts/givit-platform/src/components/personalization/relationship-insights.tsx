  ...
  return (
    <section className="container py-8 md:py-12">
      ...
      <div className="stagger-children grid gap-4 sm:grid-cols-3">
        <div className="slide-up flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-4 opacity-0">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-givit-sand">
            <UserRound className="h-5 w-5 text-givit-ember" />
          </div>
          <div>
            <p className="font-serif text-xl font-bold text-givit-ink">{insight.peopleCount}</p>
            <p className="text-xs text-muted-foreground">{insight.peopleCount === 1 ? "person" : "people"} remembered</p>
          </div>
        </div>
        <div className="slide-up flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-4 opacity-0">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-givit-sand">
            <Brain className="h-5 w-5 text-givit-ember" />
          </div>
          <div>
            <p className="font-serif text-xl font-bold text-givit-ink">{insight.interestCount}</p>
            <p className="text-xs text-muted-foreground">interests known</p>
          </div>
        </div>
        <div className="slide-up flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-4 opacity-0">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-givit-sand">
            <Heart className="h-5 w-5 text-givit-ember" />
          </div>
          <div>
            <p className="font-serif text-xl font-bold text-givit-ink">{insight.lovedCount}</p>
            <p className="text-xs text-muted-foreground">gifts loved so far</p>
          </div>
        </div>
      </div>
      ...
      {insight.upcoming.length > 0 && (
        <div className="mt-4 rounded-2xl border border-border/60 bg-card p-4">
          <p className="mb-2.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-givit-ember">
          ...
          <div className="space-y-2">
            {insight.upcoming.map((occ, i) => (
              <Link
                ...
                className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-sm transition hover:bg-muted/50"
              >
                <span className="text-foreground"><span className="font-semibold">{occ.name}</span>'s {occ.label.toLowerCase()}</span>
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${occ.daysUntil <= 14 ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>
      ...
      {insight.needsDetail.length > 0 && (
        <div className="mt-4 flex items-start gap-2.5 rounded-2xl border border-dashed border-givit-ember/40 bg-givit-sand/40 p-4">
          <Brain className="mt-0.5 h-4 w-4 shrink-0 text-givit-ember" />
