...
function PersonCard({ person }: { person: Person }) {
  ...
  const urgency = daysUntil == null ? "" : daysUntil <= 14 ? "bg-destructive/10 text-destructive" : daysUntil <= 42 ? "bg-warning/10 text-warning" : "bg-success/10 text-success";

  return (
    <button
      type="button"
      onClick={() => navigate(`/gift?q=${encodeURIComponent(buildShopQuery(person, upcoming))}`)}
      className="slide-up group flex flex-col items-start gap-4 rounded-2xl bg-card p-5 text-left opacity-0 shadow-sm shadow-black/[0.03] transition-all hover:-translate-y-0.5 hover:shadow-lg sm:p-6"
    >
